...
function countValidPasswords1(passwordList) {
	let numValid = 0;
	passwordList.forEach(password=>{
		const letterInstances = password[2].count(password[1][0]);
		if ( letterInstances >= parseInt(password[0].split("-")[0])
			&& letterInstances <= parseInt(password[0].split("-")[1]) ) {
			numValid++;
		}
	});
	return numValid;
...
function countValidPasswords2(passwordList) {
	let numValid = 0;
	passwordList.forEach(password=>{
		if (( password[2][parseInt(password[0].split("-")[0])-1]
			+password[2][parseInt(password[0].split("-")[1])-1] ).count(password[1][0])===1) {
			numValid++;
		}
	});
	return numValid;
...
const input = `13-17 s: ssssssssssssgsssj
7-9 p: pnlzhcppvl
5-6 z: zzbwrv
...
console.log(countValidPasswords1(interpretInput(input)));
console.log(countValidPasswords2(interpretInput(input)));