function interpretInput(rawInput) {
	const passportsSeparateFields = rawInput.split("\n\n").map( passport=>passport.split("\n")
		.join(',').split(" ").join(',').split(',').map(field=>field.split(":")) );
	let passportsList = [];
	passportsSeparateFields.forEach(passport=>{
		let passportDict = {};
		passport.forEach(field=>{passportDict[field[0]]=field[1]});
		passportsList.push(passportDict);
	});
	return passportsList;
}

function checkRequiredFields(passportsList) {
	const fullPassportsList = passportsList.filter((passport)=>{
		if (["byr", "iyr", "eyr", "hgt", "hcl", "ecl",
			"pid"].every(field=>Object.keys(passport).includes(field))) {
			return true;
		}
	});
	return fullPassportsList;
}

function checkHairChar(hairChar) {
	const hairChars = "0123456789abcdef";
	return hairChars.includes(hairChar);
}

function checkValidValues(fullPassportsList) {
	const validPassportsList = fullPassportsList.filter((passport)=>{
		const shouldBeNumeric1 = passport["byr"]+passport["iyr"]+passport["eyr"];
		const shouldBeNumeric2 = passport["hgt"].slice(0,-2)+passport["pid"];
		if ( shouldBeNumeric1.toString()!=parseInt(shouldBeNumeric1).toString()
			|| shouldBeNumeric2.toString()!=parseInt(shouldBeNumeric2).toString() ) {
			return false;
		}
		...
		if (passport["hgt"].slice(-2)==="cm") {
			if (!( passport["hgt"].slice(0,-2)>=150
				&& passport["hgt"].slice(0,-2)<=193 )) {return false;}
		} else if (passport["hgt"].slice(-2)==="in") {
			if (!( passport["hgt"].slice(0,-2)>=59
				&& passport["hgt"].slice(0,-2)<=76 )) {return false;}
		} else {return false;}
		if (!( passport["hcl"][0]==="#" && passport["hcl"].length===7
			&& passport["hcl"].slice(1).split("").every(checkHairChar) )) {return false;}
		if (!["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].includes(passport["ecl"])) {
			return false;
		}
		if (!(String(passport["pid"]).length===9)) {return false;}
		return true;
	});
	return validPassportsList;
}
...
console.log(checkRequiredFields(interpretInput(input)).length);
console.log(checkValidValues(checkRequiredFields(interpretInput(input))).length);
