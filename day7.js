function interpretInput(rawInput) {
	...
	inputLines.forEach((line)=>{
		let contentsDict = {};
		const halves = line.split(` contain `);
		const container = halves[0].split(" ").slice(0,2).join(" ");
		const contents = halves[1].split(", ").map(bagtype=>bagtype.split(" "));
		if (contents[0][0] != "no") {
			contents.forEach(content=>contentsDict[content.slice(1,3)
				.join(" ")] = content[0]);
			bagDict[container] = contentsDict;
		} else {
			bagDict[container] = {};
		}
	});
...
function countContainers(bagType, bagDict) {
	let potentialContainers = [], toGetContainersOf = [bagType];
	while (toGetContainersOf.length > 0) {
		toGetContainersOf.forEach((bagTypeToCheck)=>{
			Object.keys(bagDict).forEach((bag)=>{
				if ( Object.keys(bagDict[bag]).includes(bagTypeToCheck)
					&& !(potentialContainers).includes(bag) ) {
					potentialContainers.push(bag);
					toGetContainersOf.push(bag);
				}
			});
			const indexToRemove = toGetContainersOf.indexOf(bagTypeToCheck);
			toGetContainersOf.splice(indexToRemove, 1);
		});
	}
	return potentialContainers.length;
}

function countContained(bagType, bagDict) {
	let containedCount = 0;
	Object.keys(bagDict[bagType]).forEach((bag)=>{
		containedCount += bagDict[bagType][bag]*(1+countContained(bag, bagDict))
	});
...
console.log(countContainers(`shiny gold`, interpretInput(input)));
console.log(countContained(`shiny gold`, interpretInput(input)));