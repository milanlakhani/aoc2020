Array.prototype.count=function(c) {
	let total=0, pos=0;
	for (pos;pos<this.length;pos++) {
		if (this[pos]===c) {total++;}
	};
	return total;
}

function roundOfAdjacentDecisions(seatingLayout) {
	let rowIndex = -1;
	return seatingLayout.map((row)=>{
		rowIndex++;
		let columnIndex = -1;
		return row.map((seat)=>{
			columnIndex++;
			...
			[rowIndex-1, rowIndex, rowIndex+1].forEach(r=>{
				[columnIndex-1, columnIndex, columnIndex+1].forEach(c=>{
					adjacentIndexes.push({R: r, C: c});
				});
			});
			adjacentIndexes.splice(4, 1);
			const adjacentSeats = adjacentIndexes.map((seatIndex)=>{
				return ( seatingLayout[seatIndex[`R`]] ?
					seatingLayout[seatIndex[`R`]][seatIndex[`C`]] : `.` );
			});
			if (seat === `L`) {
				return adjacentSeats.includes(`#`) ? `L` : `#`;
			}
			if (seat === `#`) {
				return adjacentSeats.count(`#`) < 4 ? `#` : `L`;
			}
		});
	});
}

function roundOfVisibleDecisions(seatingLayout) {
	let rowIndex = -1;
	return seatingLayout.map((row)=>{
		rowIndex++;
		let columnIndex = -1;
		return row.map((seat)=>{
			...
			[-1, 0, 1].forEach(r=>{
				[-1, 0, 1].forEach(c=>{
					directions.push({R: r, C: c});
				});
			});
			directions.splice(4, 1);
			const visibleSeats = directions.map((direction)=>{
				let multiplier = 1;
				while (seatingLayout[multiplier*direction[`R`]+rowIndex]) {
					if ([`L`,`#`, undefined].includes( seatingLayout[
						multiplier*direction[`R`]+rowIndex][
						multiplier*direction[`C`]+columnIndex] )) {
						break;
					}
					else {
						multiplier++;
				...
				// Thought there was an easy way to have backup return values in JavaScript
				try {
					return ( seatingLayout[multiplier*direction[`R`]+rowIndex][
						multiplier*direction[`C`]+columnIndex] );
				} catch (err) {
					return `.`;
				}
			...
			if (seat === `L`) {
				return visibleSeats.includes(`#`) ? `L` : `#`;
			}
			if (seat === `#`) {
				return visibleSeats.count(`#`) < 5 ? `#` : `L`;
			}
		});
	});
}

function countFinalOccupants(seatingLayout, decisionRoundFunc) {
	let rounds = 0;
	let oldSeatingLayout = seatingLayout;
	let newSeatingLayout = decisionRoundFunc(oldSeatingLayout);
	while (newSeatingLayout != oldSeatingLayout && rounds < 110) {
		oldSeatingLayout = newSeatingLayout;
		newSeatingLayout = decisionRoundFunc(oldSeatingLayout);
		rounds++;
	}
	let finalOccupants = 0;
	newSeatingLayout.forEach(row=>finalOccupants+=row.count("#"));
	return finalOccupants;
}
...
// Misread the question to mean any visible seat not just 8 directions, may as well keep solution
function gcd(x, y) {
	return y===0?x:gcd(y, x%y);
}
function roundOfDecisions3(seatingLayout) {
	let rowIndex = -1;
	return seatingLayout.map((row)=>{
		rowIndex++;
		let columnIndex = -1;
		return row.map((seat)=>{
			columnIndex++;
			if (seat === `.`) {return `.`;}
			let visiblePositions = [], r = 0, c = 0;
			for (r=0; r<seatingLayout.length; r++) {
				for (c=0; c<row.length; c++) {
					if (gcd(Math.abs(rowIndex-r), Math.abs(columnIndex-c)) === 1) {
						visiblePositions.push({R: r, C: c});
					} else {console.log([r, c, rowIndex, columnIndex]);}
				}
			}
			const visiblePeople = visiblePositions.filter((position)=>{
				return seatingLayout[position[`R`]][position[`C`]] === `#`;
			});
			if (seat === `L`) {
				return visiblePeople.length > 0 ? `L` : `#`;
			}
			if (seat === `#`) {
				return visiblePeople.length < 5 ? `#` : `L`;
			}
		});
	});
}
...
// console.log(interpretInput(input)[0]);
console.log(countFinalOccupants(interpretInput(input), roundOfAdjacentDecisions));
console.log(countFinalOccupants(interpretInput(input), roundOfVisibleDecisions));
