function newMaths(expression) {
	let value = expression;
	while ([`+`,`*`].every(symbol=>value.includes(symbol))) {
		let cutoffPoint = value.indexOf(` `, value.indexOf(` `, value.indexOf(` `)+1)+1);
		value = eval(value.slice(0,cutoffPoint)).toString()
			+value.slice(cutoffPoint);
	}
	return eval(value).toString();
}

function sumMathsHmwk(mathsLines, differentMaths) {
	const mathsAnswers = mathsLines.map((line)=>{
		while (line.includes('(')) {
			let charIndex=0, lastOpenIndex=0, modifiedLine=line, charsRemoved=0, lastOpenModified=true;
			for (charIndex;charIndex<line.length;charIndex++) {
				if (line[charIndex] === `(`) {
					lastOpenIndex = charIndex;
					lastOpenModified = false;
				}
				if (line[charIndex] === `)` && !lastOpenModified) {
					const bracketsValue = differentMaths(line.slice(lastOpenIndex+1,charIndex));
					modifiedLine = modifiedLine.slice(0,lastOpenIndex-charsRemoved)
						+ bracketsValue + line.slice(charIndex+1);
					lastOpenModified = true;
					charsRemoved += charIndex-lastOpenIndex+1-bracketsValue.length;
				}
			}
			line = modifiedLine;
		}
		return parseInt(differentMaths(line));
	});
	return mathsAnswers.reduce((x,y)=>x+y, 0);
}

function advancedMaths(expression) {
	let value = expression;
	while ([`+`,`*`].every(symbol=>value.includes(symbol))) {
		const plusIndex = value.indexOf(`+`);
		let spaceBeforeIndex = -1, spaceBeforeFound = false;
		while (!spaceBeforeFound) {
			if (value.indexOf(` `, spaceBeforeIndex+1) === plusIndex-1) {
				spaceBeforeFound = true;
			} else {
				spaceBeforeIndex = value.indexOf(` `, spaceBeforeIndex+1);
			}
		}
		const spaceAfterIndex = value.indexOf(` `, plusIndex+2);
		if (plusIndex === value.indexOf(` `)+1) {
			value = eval(value.slice(0, spaceAfterIndex)).toString()
				+value.slice(spaceAfterIndex);
		} else if (spaceAfterIndex != -1) {
			value = value.slice(0, spaceBeforeIndex+1)
				+eval(value.slice(spaceBeforeIndex+1, spaceAfterIndex)).toString()
				+value.slice(spaceAfterIndex);
		} else {
			value = value.slice(0, spaceBeforeIndex+1)
				+eval(value.slice(spaceBeforeIndex+1)).toString();
		}
	}
	return eval(value).toString();
}
...
mathsHmwk = `8 * 6 * 6 * 4 * (8 + 5 * 2 + 5 + 6 + 3) * 4
...
console.log(sumMathsHmwk(interpretInput(mathsHmwk), newMaths));
console.log(sumMathsHmwk(interpretInput(mathsHmwk), advancedMaths));
